// TeamTweaker — Popup Logic
// Reads/writes settings to chrome.storage.sync and updates UI.
...
// Throttled storage write — prevents hitting Chrome's
// MAX_WRITE_OPERATIONS_PER_MINUTE quota (120/min) when color pickers
// or sliders fire rapid "input" events.
const _pendingWrites = {};
let _writeTimer = null;
const WRITE_INTERVAL_MS = 600; // flush at most ~100 writes/min
...
// Sub-option visibility mapping: toggle id → sub-option container id
const SUB_OPTIONS = {
...
/** Monospace stack for Matrix preset; keep identical in panel and `<option value>` in HTML. */
const TERMINAL_FONT_STACK =
  'Consolas, "Cascadia Mono", "Cascadia Code", "Courier New", monospace';

/** Linux-style dev / distro monospace stack; keep identical in panel and `<option value>` in HTML. */
...
/** Named palettes: section gradient anchor + accent + unread/active highlights (keep in sync with sidepanel/panel.js). */
...
document.addEventListener("DOMContentLoaded", () => {
  // Detect full-tab mode (window wider than a normal popup)
  const isFullTab = window.outerWidth > 400;

  if (isFullTab) {
    document.body.classList.add("tf-full-tab");
  }

  // Hide the "open in tab" button when already running as a full tab
  ...
  // "Unread Only" button — opens the side panel (or tab as fallback)
  const openUnreadBtn = document.getElementById("openUnreadBtn");
  if (openUnreadBtn) {
    openUnreadBtn.addEventListener("click", async () => {
      try {
        const win = await chrome.windows.getCurrent();
        await chrome.sidePanel.open({ windowId: win.id });
      } catch {
        chrome.tabs.create({ url: chrome.runtime.getURL("unread/unread.html") });
      }
    });
  }
  ...
  chrome.storage.sync.get(DEFAULTS, (settings) => {
    ...
    // Render section color entries
    renderSectionList(settings.sectionColorsJSON);

    // Update accent color on root for the popup itself
    document.documentElement.style.setProperty("--accent", settings.accentColor);

    // Set active color preset dot
  ...
  // Prevent clicks on inline controls from toggling the parent <label> checkbox
  ...
  // Unread background color picker
  ...
  // Active chat background color picker
  ...
  // Color clear buttons — set value to "" and mark picker as cleared
...
function updateSubOptions() {
  ...
  // Unread inline options (inside the toggle row, not a separate sub-option)
...
function parseSectionValue(value) {
  ...
  if (value && typeof value === "object") {
    return { bg: value.bg || "#6264a7", text: value.text || "#ffffff" };
  }
  return { bg: "#6264a7", text: "#ffffff" };
}
...
// ---- Emoji Picker (keep EMOJI_OPTIONS identical in sidepanel/panel.js) ----
...
function initAllEmojiPickers(settings) {
  ...
  buildEmojiPickerOnce("pinnedEmojiPickerBtn", "pinnedEmojiPickerDropdown", "pinnedEmoji", null);
